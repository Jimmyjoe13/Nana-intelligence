...
document.addEventListener("DOMContentLoaded", function () {
  console.log("✅ Nana Intelligence JS Loaded");
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('focusin', () => {
      if (!formStarted && typeof gtag === 'function') {
        formStarted = true;
        gtag('event', 'form_start', { 'form_name': 'contact_form', 'transport': 'beacon' });
      }
    }, { once: true });
  ...
  // =========================================
  console.log("✅ GA4 Click Tracking Initialized");
  document.addEventListener("click", function (e) {
    ...
    if (typeof gtag === 'function') {
      ...
      if (contactKeywords.some(keyword => text.toLowerCase().includes(keyword))) {
        gtag('event', 'cta_click', { 'cta_text': text, 'section_id': section, 'transport': 'beacon' });
      }

      // 2. Outbound
      if (href.startsWith("http") && !href.includes(window.location.hostname)) {
        gtag('event', 'outbound_click', { 'link_url': href, 'link_text': text, 'transport': 'beacon' });
      }

      // 3. Contact Intent
      if (href.startsWith("mailto:") || href.includes("malt.fr")) {
        gtag('event', 'contact_intent', { 'type': href.startsWith("mailto:") ? 'email' : 'malt', 'transport': 'beacon' });
      }
    }
  });
